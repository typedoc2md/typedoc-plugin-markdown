...
- Each member is now output to its own file by default. See [`--outputFileStrategy`](/options#outputfilestrategy). To achieve the same output as v3 (wherby only Classes, Enums and Interfaces have their own file), use [`--membersWithOwnFile`](/options#memberswithownfile)
