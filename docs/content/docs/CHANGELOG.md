...
From a consumer perspective there should be no direct breaking changes, but there are several changes that may effect the output of this plugin. Please see the full [TypeDoc changelog 0.28]() for additional information.

### Architectural Changes

- This plugin now utilizes TypeDoc router implementation. This replaces the now deprecated `outputFileStrategy` option. `outputFileStrategy` is still supported for now, but will now point to the "kind" and "module" router keys respectively.
- Please note that separate strategies for output generation in different packages is no longer possible with this new architecture.
