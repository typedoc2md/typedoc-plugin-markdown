...
import { DeclarationOption, DeclarationReflection } from 'typedoc';
import {
  MarkdownApplication,
  MarkdownPageEvent,
...
export function load(app: MarkdownApplication) {
  ...
  app.renderer.on(MarkdownPageEvent.BEGIN, (page: MarkdownPageEvent) => {
    ...
    const comment = getComment(page.model as DeclarationReflection);
  ...
  app.renderer.on(MarkdownPageEvent.END, (page: MarkdownPageEvent) => {
    if (page.frontmatter && Object.keys(page.frontmatter)?.length) {
      const yamlStringifyOptions = app.options.getValue('yamlStringifyOptions');
