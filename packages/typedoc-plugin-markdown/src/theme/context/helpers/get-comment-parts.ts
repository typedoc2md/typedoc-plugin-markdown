...
export function getReflectionUrl(
  ...
  const anchor = context.router.getAnchor(reflection);
  if (anchor && context.helpers.useUniqueAnchor(reflection)) {
    return `${context.urlTo(reflection).replace(/#.*$/, '')}#${context.router.getUniqueAnchor(reflection)}`;
  }
