...
export class MarkdownThemeContext {
  ...
  urlTo(reflection: Reflection) {
    ...
    if (this.router.hasUrl(reflection)) {
      ...
      if (anchor && this.helpers.useUniqueAnchor(reflection)) {
        return `${this.router.relativeUrl(this.page.model, reflection).replace(/#.*$/, '')}#${this.router.getUniqueAnchor(reflection)}`;
      }
