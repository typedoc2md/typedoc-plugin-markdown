    ...
    if (
      ...
      if (
        ...
        project.groups?.forEach((group) => {
          this.buildUrlsFromGroup(
            group as unknown as DeclarationReflection,
            groupProps,
          );
        });
      } else {
    ...
    if (mapping) {
      ...
      if (
        ...
        reflection.groups?.forEach((group) => {
          (group as any).parent = reflection;
          this.buildUrlsFromGroup(group as unknown as DeclarationReflection, {
            parentUrl: urlPath,
            directory: 'xxx',
            outputFileStrategy: urlOptions.outputFileStrategy,
          });
  ...
  private getUrlPath(reflection: DeclarationReflection, urlOption: UrlOption) {
    ...
    const dir = () => {
      ...
      if (reflection.kind === ReflectionKind.Namespace) {
        const baseDir = urlOption.category
          ? `${urlOption.category}/${urlOption.directory}`
          : urlOption.directory;
