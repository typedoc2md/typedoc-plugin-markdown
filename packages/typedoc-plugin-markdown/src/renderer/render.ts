...
export async function render(
  ...
  for (const page of pages) {
    await renderDocument(renderer, outputDirectory, page, formatWithPrettier);
  }
...
function prepareRouter(renderer: Renderer) {
  ...
  if (
    !renderer.application.options.isSet('router') &&
    renderer.application.options.isSet('outputFileStrategy')
  ) {
    const outputFileStrategy =
      renderer.application.options.getValue('outputFileStrategy');
    routerOption = outputFileStrategy === 'modules' ? 'module' : 'kind';
  }
  ...
  if (!ctor) {
    renderer.application.logger.error(
      i18n.router_0_is_not_defined_available_are_1(
        routerOption,
        ['kind', 'module', 'category', 'group'].join(', '),
      ),
    );
    return false;
  }
