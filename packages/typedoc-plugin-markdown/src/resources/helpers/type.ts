...
export default function (context: MarkdownThemeContext) {
  Handlebars.registerHelper(
    ...
    function (
      ...
      if (this instanceof Array && this[0] instanceof SignatureReflection) {
        return getFunctionType(this, collapse);
      }
    ...
    if (model.indexSignature || model.children) {
      ...
      if (declarationIndexSignature) {
        const key = declarationIndexSignature.parameters
          ...
          : '';
        const obj = Handlebars.helpers.type.call(
  ...
  function getReferenceType(model: ReferenceType, emphasis: boolean) {
    const externalUrl = context.attemptExternalResolution(model.getSymbol());
